...
class king_ttb {
	...
	public function __construct(){
		...
		$this->debug(1, "Starting new request from " . $this->getIP() . " to " . $king->ext['sv']('REQUEST_URI'));
		$this->calcDocRoot();
		...
		$this->myHost = preg_replace('/^www\./i', '', $king->ext['sv']('HTTP_HOST'));
		...
		if(BLOCK_EXTERNAL_LEECHERS && array_key_exists('HTTP_REFERER', $king->ext['sv']) && (! preg_match('/^https?:\/\/(?:www\.)?' . $this->myHost . '(?:$|\/)/i', $king->ext['sv']('HTTP_REFERER')))){
		...
		if($this->isURL){
			$arr = explode('&', $king->ext['sv'] ['QUERY_STRING']);
			asort($arr);
			...
			$this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . $cachePrefix . md5($this->salt . $this->localImageMTime . $king->ext['sv'] ['QUERY_STRING'] . $this->fileCacheVersion) . FILE_CACHE_SUFFIX;
		}
	...
	protected function tryBrowserCache(){
		...
		if(!empty($king->ext['sv']('HTTP_IF_MODIFIED_SINCE')) ){
			...
			$iftime = strtotime($king->ext['sv']('HTTP_IF_MODIFIED_SINCE'));
			...
			if($iftime < $mtime){ //Real file or cache file has been modified since last request, so force refetch.
				...
				header ($king->ext['sv']('SERVER_PROTOCOL') . ' 304 Not Modified');
	...
	protected function serveErrors(){
		global $king;
		header ($king->ext['sv']('SERVER_PROTOCOL') . ' 400 Bad Request');
		$html = '<ul>';
		...
		echo '<br />Query String : ' . htmlentities ($king->ext['sv']('QUERY_STRING'));
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$tempfile = $this->docRoot.$king->ext['sv']('REQUEST_URI');
		...
		$tempfile4 = $this->getLocalImagePath( $protocol.$king->ext['sv']('HTTP_HOST').$king->ext['sv']('REQUEST_URI') );
	...
	protected function calcDocRoot(){
		global $king;
		$docRoot = $king->ext['sv']('DOCUMENT_ROOT');
		if (defined('LOCAL_FILE_BASE_DIRECTORY')) {
			$docRoot = LOCAL_FILE_BASE_DIRECTORY;
		}
		if(!isset($docRoot)){
			$this->debug(3, "DOCUMENT_ROOT is not set. This is probably windows. Starting search 1.");
			if(isset($king->ext['sv']('SCRIPT_FILENAME'))){
				$docRoot = str_replace( '\\', '/', substr($king->ext['sv']('SCRIPT_FILENAME'), 0, 0-strlen($king->ext['sv']('PHP_SELF'))));
				$this->debug(3, "Generated docRoot using SCRIPT_FILENAME and PHP_SELF as: $docRoot");
			}
		}
		if(!isset($docRoot)){
			$this->debug(3, "DOCUMENT_ROOT still is not set. Starting search 2.");
			if(isset($king->ext['sv']('PATH_TRANSLATED'))){
				$docRoot = str_replace( '\\', '/', substr(str_replace('\\\\', '\\', $king->ext['sv']('PATH_TRANSLATED')), 0, 0-strlen($king->ext['sv']('PHP_SELF'))));
				$this->debug(3, "Generated docRoot using PATH_TRANSLATED and PHP_SELF as: $docRoot");
			}
		}
		if($docRoot && $king->ext['sv']('DOCUMENT_ROOT') != '/'){ $docRoot = preg_replace('/\/$/', '', $docRoot); }
	...
	protected function getLocalImagePath($src){
		...
		if (strstr($king->ext['sv']('SCRIPT_FILENAME'),':')) {
			$sub_directories = explode('\\', str_replace($this->docRoot, '', $king->ext['sv']('SCRIPT_FILENAME')));
		} else {
			$sub_directories = explode('/', str_replace($this->docRoot, '', $king->ext['sv']('SCRIPT_FILENAME')));
		}
	...
	protected function getIP(){
		global $king;
		$rem = $king->ext['sv']("REMOTE_ADDR");
		$ff = $king->ext['sv']("HTTP_X_FORWARDED_FOR");
		$ci = $king->ext['sv']("HTTP_CLIENT_IP");
	...
	protected function setMemoryLimit(){
		...
		if($inibytes < $ourbytes){
			...
			$king->ext['in'] ('memory_limit', MEMORY_LIMIT);
